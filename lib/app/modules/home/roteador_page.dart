import 'package:get/get.dart';
import 'package:flutter/material.dart';
import 'package:catalogo_produto_poc/app/modules/home/home_page.dart';
import 'package:catalogo_produto_poc/app/modules/usuario/page/usuario_form_page.dart';
import 'package:catalogo_produto_poc/app/core/widget/widget_loading_page.dart';
import 'package:catalogo_produto_poc/app/modules/usuario/controller/usuario_controller.dart';

// Implementei usando GetX, Obx e StreamBuilder para gerenciar o estado do usuário
// GetX
class RoteadorPage extends StatelessWidget {
  const RoteadorPage({super.key});
  @override
  Widget build(BuildContext context) {
    return GetX<UsuarioController>(
      init: Get.find<UsuarioController>(),
      builder: (controller) {
        if (controller.isLoading) {
          return WidgetLoadingPage(
            label: 'Carregando...',
            labelColor: Get.theme.colorScheme.primary,
            backgroundColor: Colors.white,
          );
        }
        return controller.user != null
            ? const HomePage()
            : const UsuarioFormPage();
      },
    );
  }
}

// Obx
// class RoteadorPage extends StatelessWidget {
//   const RoteadorPage({super.key});
//   @override
//   Widget build(BuildContext context) {
//     return Obx(() {
//       final usuarioController = Get.find<UsuarioController>();
//       if (usuarioController.isLoading) {
//         return WidgetLoadingPage(
//           label: 'Carregando...',
//           labelColor: Get.theme.colorScheme.primary,
//           backgroundColor: Colors.white,
//         );
//       }
//       return usuarioController.user != null
//           ? const HomePage()
//           : const UsuarioFormPage();
//     });
//   }
// }

// StreamBuilder
// import 'package:firebase_auth/firebase_auth.dart';
// class RoteadorPage extends StatelessWidget {
//   const RoteadorPage({super.key});
//   @override
//   Widget build(BuildContext context) {
//     return StreamBuilder<User?>(
//       stream: Get.find<UsuarioController>().authState,
//       builder: (ctx, snapshot) {
//         if (snapshot.connectionState == ConnectionState.waiting) {
//           return WidgetLoadingPage(
//             label: 'Carregando...',
//             labelColor: Theme.of(context).colorScheme.primary,
//             backgroundColor: Colors.white,
//           );
//         } else {
//           return snapshot.hasData ? const HomePage() : const UsuarioFormPage();
//         }
//       },
//     );
//   }
// }
